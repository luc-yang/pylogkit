...
class QtLogHandler:
    ...
    它既服务于 `attach_qt()`，也可被 `QtLoggingHandler` 复用。
    ...
    def __init__(
        ...
        format_string: str = "{time} | {level:<8} | {message}",
    ) -> None:
        """
        初始化 Qt 日志处理器
    ...
    def emit(self, record: dict[str, Any] | LogRecord) -> None:
        """
        处理并发射日志记录
        ...
        Args:
            record: 日志记录，可以是 loguru 的记录字典或 logging.LogRecord 对象
        """
        try:
            # 解析记录为统一格式
    ...
    def _parse_record(
        self, record: dict[str, Any] | LogRecord
    ) -> dict[str, Any]:
        """
        解析日志记录为统一格式
        ...
        if isinstance(record, dict):
            level_value = record.get("level", "INFO")
            if isinstance(level_value, dict):
                level_name = level_value.get("name", "INFO")
            elif hasattr(level_value, "name"):
                level_name = str(level_value.name)
            else:
...
class QtLoggingHandler(Handler):
    ...
    ```python
    import logging
    from pylogkit.qt_integration import QtLoggingHandler, LogSignalEmitter
